...
class PolicyValueNet:
    ...
    def __init__(self, model_file=None, use_gpu=True, device = 'cuda'):
        ...
        print("检查设备支持：",next(self.policy_value_net.parameters()).device)  # 应该输出 "cuda"
   ...
   # 输入棋盘，返回每个合法动作的（动作，概率）元组列表，以及棋盘状态的分数
    ...
    def train_step(self, state_batch, mcts_probs, winner_batch, lr=0.002):
        ...
        # 打印训练过程中的详细信息
        print(f"Loss: {loss_item:.4f} | "
              f"Value Loss: {value_loss_item:.4f} | "
              f"Policy Loss: {policy_loss_item:.4f} | "
              f"Entropy: {entropy_item:.4f} | "
              f"Learning Rate: {lr:.6f}")

        return loss_item, entropy_item
